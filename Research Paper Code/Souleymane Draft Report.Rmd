...
# Set working directory to the location of the open .Rmd file
...
numeric_data <- project_model_data_final_2 %>% select_if(is.numeric) %>% select(-c(avg_cyclone_freq, avg_temp_anomaly, avg_max_wind_speed))
corr_matrix <- cor(numeric_data, use = "complete.obs")
corrplot(corr_matrix, method = "color", type = "upper", tl.cex = 0.7, number.digits = 1, number.cex = 0.6)
```


### 4.6.3 Pairwise Scatter Plots: Cost/Delay vs Key Features
```{r scatter-cost-delay}
...
### 4.6.5 Relationship between project_profile and categorical variables
```{r}
library(ggplot2)
library(dplyr)
...
## 4.7 Predicting cost_class and delay_class separately
...
## 4.8 Implementing modeling to predict project_profile
...
# Load udpdated project_model_data_final with each line representing unique record of unique project
...
#project_model_data_final_updated[, numeric_cols] <- scale(project_model_data_final_updated[, numeric_cols])
...
levels(project_model_data_final_updated$weather_risk_category) <- c("Low", "Moderate", "High")[cluster_order]
...
## 5.2 [Same as 4.7] Predicting cost_class and delay_class separately
```{r Predicting cost_class and delay_class separately 2}
# ---- Setup for Predicting cost_class and delay_class separately ----
library(caret)
library(randomForest)
library(rpart.plot)
...
cost_model_data_updated <- project_model_data_final_updated %>%
  select(-c(avg_cyclone_freq, avg_temp_anomaly, avg_max_wind_speed, status_combined, cost_class, fms_id, project_profile, cost_cat_environmental, cost_cat_housing, cost_cat_infrastructure, cost_cat_miscellaneous_other,  cost_cat_parks_recreation, cost_cat_public_buildings, cost_cat_transportation))
...
delay_model_data_updated <- project_model_data_final_updated %>%
  select(-c(avg_cyclone_freq, avg_temp_anomaly, avg_max_wind_speed, status_combined, delay_class, fms_id, project_profile,  delay_cat_contractor_issues, delay_cat_funding_financial_issues, delay_cat_legal_contractual_issues, delay_cat_pending_governmental_approvals, delay_cat_resource_technology_availability,  delay_cat_scope_or_design_changes,   delay_cat_unforeseen_site_conditions))
...
## 5.3 [Same as 4.8] Implementing modeling to predict project_profile
```{r Implementing modeling on the final data 2}
...
project_model_data_final_all_updated <- project_model_data_final_updated %>%
  select(-c(avg_cyclone_freq, avg_temp_anomaly, avg_max_wind_speed, status_combined, cost_class, delay_class, fms_id, project_profile))
...
# Convert target and categorical predictors to factors
project_model_data_final_all_updated$project_profile <- as.factor(project_model_data_final_updated$project_profile)
...
# Convert character columns to factors
project_model_data_final_all_updated[categorical_vars] <- lapply(project_model_data_final_all_updated[categorical_vars], as.factor)

# Partition data into training and testing sets
set.seed(123)
train_index <- createDataPartition(project_model_data_final_all_updated$project_profile, p = 0.8, list = FALSE)
...
numeric_data <- project_model_data_final_updated %>% select_if(is.numeric) %>% select(-c(avg_cyclone_freq, avg_temp_anomaly, avg_max_wind_speed))
corr_matrix <- cor(numeric_data, use = "complete.obs")
